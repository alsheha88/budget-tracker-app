...
import type { CreateBillData, EditBillData, MarkAsPaid } from "../../../schemas/bills/bills.js";
...
export const dbMarkAsPaid = async (id:string, userId:string) => {
	return prisma.bill.updateMany({ where: { id, userId }, data: { paidAt: new Date() } });
}
export const dbGetBills = async (userId: string) => {
	return await prisma.bill.findMany({ where: { userId }, include: {category: true, account: true} });
};
export const dbGetBill = async (userId: string, id: string) => {
	return await prisma.bill.findFirst({ where: { userId, id }, include: {category: true, account: true} });
};
